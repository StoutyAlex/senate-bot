...
export interface WallsBundleProps {
    buildName: (name: string) => string
    rconPassword: string
    serverSecurityGroup: SenateMCSecurityGroup
}

export class WallsBundle extends cdk.Construct {
    public bucket: s3.Bucket

    constructor(scope: cdk.Construct, id: string, props: WallsBundleProps) {
        super(scope, id)

        const { buildName, serverSecurityGroup, rconPassword } = props

        const serverFilesDir = 'server-files'
        const serverBundleDir = 'server-bundle'
